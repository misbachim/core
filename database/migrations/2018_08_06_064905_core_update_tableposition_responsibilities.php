...
class CoreUpdateTablepositionResponsibilities extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('position_responsibilities', function (Blueprint $table) {
            if (!Schema::hasColumn('position_responsibilities', 'eff_begin')) {
                $table->date('eff_begin')->default('2017-01-01');
            }
            if (!Schema::hasColumn('position_responsibilities', 'eff_end')) {
                $table->date('eff_end')->default('9999-12-31');
            }
            if (!Schema::hasColumn('position_responsibilities', 'is_appraisal')) {
