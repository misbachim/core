...
class SearchQueryBuilder
{
    ...
    public function leftJoin(...$leftJoinClauses)
    {
        ...
        if ($leftJoinClauses[0] == 'person_co_fields') {

            info('($this->whereClauses');
            info(print_r($this->whereClauses, true));

            // modify column reference (custom object)
            $this->whereClausesTemp = [];
            foreach ($this->selectedFields as $key => $sField) {
                $sFieldArr = explode(' ', trim($sField));
                if ($sFieldArr[0] == 'person_co_fields.value') {

                    $fieldCode = explode(" ", $sFieldArr[2]);

                    $customObjectFieldDao = new CustomObjectFieldDao($requester);
                    $asName = $customObjectFieldDao->getNameById($fieldCode);
                    $columnAsName = $asName[0]->name;

                    $tableName = "person_co_fields_$key";
                    $joinedTable = "person_co_fields as $tableName";
                    $joinOnTable = "$tableName.person_co_id";
                    $selectedColumn =  "$tableName.value as  \"$columnAsName\"";
                    $whereRef = "$tableName.co_field_id";

                    $this->selectedFields[$key] = $selectedColumn;
                    $leftJoinClauses[0] = $joinedTable;
                    $leftJoinClauses[1] = $joinOnTable;

                    $this->builder = $this->builder->leftJoin(...$leftJoinClauses);

                    array_push($this->whereClausesTemp, [$whereRef, '=', $fieldCode]);

                    // foreach ($this->whereClauses as $key => $value) {
                    //     if ($value[0] == 'person_co_fields.value') {
                    //         array_push($this->whereClausesTemp, ["$tableName.value", $value[1], $value[2]]);
                    //     }
                    // }
                }
            }

            // foreach ($this->whereClauses as $value) {
            //     if ($value[0] != 'person_co_fields.value') {
            //         array_push($this->whereClausesTemp, $value);
            //     }
            // }
            // $this->whereClauses = $this->whereClausesTemp;
        } else {
            $this->builder = $this->builder->leftJoin(...$leftJoinClauses);
        }
    ...
    private function initSelectedFields()
    {
        ...
        foreach ($this->searchData['selectedFields'] as $fieldCode) {
            ...
            if (is_int($fieldCode)) {
                ...
                array_push($this->selectedFields, "$column as $fieldCode");
            } else if ($fieldCode[0] == 'c') {
    ...
    private function initWhereClauses()
    {
        ...
        // special case for custom object
        // foreach ($this->searchData['selectedFields'] as $fieldCode) {
        //     if (is_int($fieldCode)) {
        //         array_push($this->whereClauses, ['person_co_fields.co_field_id', '=', $fieldCode]);
        //     }
        // }
